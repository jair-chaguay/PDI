...
def warp_rgba(img, M, out_w, out_h):
    ...
    eps = 1e-6
    bgr_out = bgr_w / (a_w[..., None] + eps)
    bgr_out = np.clip(bgr_out, 0, 255).astype(np.uint8)

    out = np.dstack([bgr_out, (a_w * 255.0).astype(np.uint8)])
    out[out[:, :, 3] == 0, :3] = 0
    return out
...
def aplicar_sombrero(frame, face, w, h):
    ...
    if eye_dist < 2:
        return frame
    ang = angle_deg(le, re)
    target_w = int(eye_dist * 2.8)
    scale = target_w / hat.shape[1]
    target_h = max(1, int(hat.shape[0] * scale))
    hat_rs = resize_rgba(hat, max(1, target_w), max(1, target_h))
    hat_rot = rotate_rgba(hat_rs, ang)
    cx = int((le[0] + re[0]) / 2)
    cy = int(fh[1] - eye_dist * 0.95)
...
def aplicar_gafas(frame, face, w, h):
    ...
    if eye_dist < 2:
        return frame
    ang = angle_deg(le, re)
    target_w = int(eye_dist * 2.6)
    scale = target_w / glasses.shape[1]
    target_h = max(1, int(glasses.shape[0] * scale))
    g_rs = resize_rgba(glasses, max(1, target_w), max(1, target_h))
    g_rot = rotate_rgba(g_rs, ang)
    cx = int((le[0] + re[0]) / 2)
    cy = int((le[1] + re[1]) / 2) + int(eye_dist * 0.10)
...
def aplicar_bigote(frame, face, w, h):
    ...
    m_rs = resize_rgba(must, max(1, target_w), max(1, target_h))
    m_rot = rotate_rgba(m_rs, ang)
    cx = nose[0]
    cy = int((nose[1] + lip[1]) / 2) + int(eye_dist * 0.08)
...
def aplicar_orejas(frame, face, w, h):
    ...
    e_rs = resize_rgba(ears, max(1, target_w), max(1, target_h))
...
def filtro_ojos_grandes(frame, face, w, h):
    ...
    eye_span = dist(le, re)
    rad = int(eye_span * 0.25)
    out = frame.copy()
    out = bulge_pinch_roi(out, le, rad, strength=0.75)
    out = bulge_pinch_roi(out, re, rad, strength=0.75)
    return out
...
def filtro_boca_grande(frame, face, w, h):
    ...
    rad = int(max(mouth_w, mouth_h) * 0.90)
    out = frame.copy()
    out = bulge_pinch_roi(out, (cx, cy), rad, strength=0.75)
    return out
...
def filtro_nariz_pequena(frame, face, w, h):
    ...
    re = lm_xy(face, LMS["right_eye"], w, h)
    eye_dist = dist(le, re)
    rad = int(eye_dist * 0.28)
    out = frame.copy()
    out = bulge_pinch_roi(out, nose, rad, strength=-0.65)
    return out
...
def filtro_cara_ancha(frame, face, w, h):
    ...
    rad = int(face_w * 0.28)
    out = frame.copy()
    out = bulge_pinch_roi(out, fl, rad, strength=0.70)
    out = bulge_pinch_roi(out, fr, rad, strength=0.70)
    return out
...
def filtro_menton_largo_alien(frame, face, w, h):
    chin = lm_xy(face, LMS["chin"], w, h)
    le = lm_xy(face, LMS["left_eye"], w, h)
    re = lm_xy(face, LMS["right_eye"], w, h)
    eye_dist = dist(le, re)
    out = frame.copy()
    out = bulge_pinch_roi(out, chin, int(eye_dist * 0.38), strength=0.70)
    out = warp_shift_circle(out, chin, int(eye_dist * 0.55), shift=int(eye_dist * 0.28))
    return out


def filtro_mejillas_infladas_chipmunk(frame, face, w, h):
    fl = lm_xy(face, LMS["face_left"], w, h)
    fr = lm_xy(face, LMS["face_right"], w, h)
    nose = lm_xy(face, LMS["nose"], w, h)
    le = lm_xy(face, LMS["left_eye"], w, h)
    ...
    eye_dist = dist(le, re)
    out = frame.copy()
    out = bulge_pinch_roi(out, fl, int(face_w * 0.22), strength=0.82)
    out = bulge_pinch_roi(out, fr, int(face_w * 0.22), strength=0.82)
    out = bulge_pinch_roi(out, nose, int(eye_dist * 0.22), strength=0.25)
    return out


def filtro_frente_grande(frame, face, w, h):
    fh = lm_xy(face, LMS["forehead"], w, h)
    chin = lm_xy(face, LMS["chin"], w, h)
    fl = lm_xy(face, LMS["face_left"], w, h)
    fr = lm_xy(face, LMS["face_right"], w, h)
    face_w = dist(fl, fr)
    out = frame.copy()
    out = bulge_pinch_roi(out, fh, int(face_w * 0.28), strength=0.78)
    out = bulge_pinch_roi(out, chin, int(face_w * 0.30), strength=-0.38)
    return out


def filtro_boca_pequena_muneco(frame, face, w, h):
    ml = lm_xy(face, LMS["mouth_left"], w, h)
    mr = lm_xy(face, LMS["mouth_right"], w, h)
    le = iris_center(face, "L", w, h)
    re = iris_center(face, "R", w, h)
    ...
    eye_span = dist(le, re)
    out = frame.copy()
    out = bulge_pinch_roi(out, (cx, cy), int(mouth_w * 0.80), strength=-0.72)
    out = bulge_pinch_roi(out, le, int(eye_span * 0.22), strength=0.38)
    out = bulge_pinch_roi(out, re, int(eye_span * 0.22), strength=0.38)
    return out
...
def filtro_smile_forzado(frame, face, w, h):
    ...
    mouth_w = dist(ml, mr)
    out = frame.copy()
    shift = int(max(4.0, eye_dist * 0.10))
    out = warp_shift_circle(out, ml, int(mouth_w * 0.55), shift=shift)
    out = warp_shift_circle(out, mr, int(mouth_w * 0.55), shift=shift)
    out = bulge_pinch_roi(
        out,
        (int((ml[0] + mr[0]) / 2), int((ml[1] + mr[1]) / 2)),
        int(mouth_w * 0.55),
        strength=0.10,
    )
    return out
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                ...
                if detected_command:
                    active_filter_index = change_filter(detected_command, active_filter_index, len(FILTROS))
            ...
            cv2.imshow("Filtros tipo redes sociales", frame)
